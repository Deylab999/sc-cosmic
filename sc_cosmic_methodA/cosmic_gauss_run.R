#' sc-COSMIC: Gaussian counterfactual inference model for single-cell RNA-seq data
#'
...
#' @param input_data matrix/sparse-matrix or data frame of log TP10K/TPM/CPM/RPKM of RNA counts, with samples along rows and genes along columns.
...
#'								propensity_knn: kNN based on propensity score with a caliper from `caliper_bound`,
#'							  svd_knn: kNN based on SVD on the data matrix
#'								propensity_svd_knn: kNN on SVD with propensity score caliper with a caliper bound from `caliper_bound`
...
cosmic_gauss_run = function(input_data,
							 sample_metadata,
							 gene_metadata,
							 select_genes,
							 context1,
	...
	if(!is.matrix(input_data) & !is.data.frame(input_data) & class(S4data_sparse)[1] != "dgCMatrix"){
	...
	for(counter1 in 1:length(select_cell_types)){
		...
		mm2 = sweep(mm, 2, colSums(mm), "/")
    	lambdas_indiv = S4data_sparse_celltype2 %*% mm2
	...
	for(counter1 in 1:length(select_cell_types)){
       ...
       for(iter_counterfac in 1:NUM_ITER_COUNTERFAC){
			...
			model_indiv2 = sweep(model_indiv, 2, colSums(model_indiv), "/")
			
			deltas_full_data = context2_fulldata - context1_fulldata
			deltas_indiv = deltas_full_data %*% model_indiv2
