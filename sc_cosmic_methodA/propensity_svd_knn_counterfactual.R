#' Find counterfactual neighbors based on Propensity + SVD scoring of cells in sc-COSMIC
#'
...
#' based approach to generate a set of nearest neighbors counterfactuals of opposite context to each cell, along their 
...
#'         `X`: A GxN matrix that same as the input data matrix.
#' @export 

propensity_svd_knn_counterfactual = function(Xmat,
										 context_vec,
										 knn_k_bound=5,
										 rank_svd = 50,
	...
	knn_out2 = try(get_knn(as.matrix(propensity_scores), k=1000, distance = "euclidean"), silent=T)
	if(class(knn_out2) == "try-error"){
	...
	for(i in 1:nrow(knn_out$idx)){
		if(context_vec2[i] == context1){
			fidx1 = knn_out$idx[i, which(knn_out2$dist[i, ] < caliper_choose*sd(propensity_scores))]
