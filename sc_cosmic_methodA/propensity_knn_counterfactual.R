#' Find counterfactual neighbors based on Propensity scoring of cells in sc-COSMIC
#'
#' The \code{propensity_knn_counterfactual} function takes in a matrix/data frame (G x N), a context metadata vector of length N 
#' comprising of two distint labels (corresponding to context1 and context2) and uses a propensity score based approach based on 
#' gradient boost to output a set of nearest neighbors counterfactuals of opposite context to each cell, along their 
...
# Xmat=S4data_sparse_celltype2
# context_vec = context_celltype_metadata2
# knn_k_bound = 5
# rank_svd = 50
...
propensity_knn_counterfactual = function(Xmat,
										 context_vec,
										 knn_k_bound=5,
										 rank_svd=50,
										 caliper_bound_vec = c(0.10, 0.15, 0.20, 0.25),
	...
	for(numm in 1:CROSSVAL_BATCH_NUM){
		bstSparse <-  xgboost(data = t(svd_out_proj_combined_t[, oo[[numm]]]), 
                    ...
                    eval_metric = "auc")
		test_ids = 1:ncol(svd_out_proj_combined_t)
	...
	knn_out = try(get_knn(as.matrix(propensity_scores), k=1000, distance = "euclidean"), silent=T)
	if(class(knn_out) == "try-error"){
		knn_out = try(get_knn(as.matrix(propensity_scores), k=1000, distance = "l2"), silent=T)
		cat("WARNING: Euclidean distance metric failed; We are using the L2 metric as distance in kNN computation \n")
	}
	if(class(knn_out) == "try-error"){
		knn_out = try(get_knn(as.matrix(propensity_scores), k=1000, distance = "ip"), silent=T)
		cat("WARNING: Euclidean distance metric failed; We are using the IP metric as distance in kNN computation \n")
	}
	if(class(knn_out) == "try-error"){
		knn_out = try(get_knn(as.matrix(propensity_scores), k=1000, distance = "cosine"), silent=T)
	...
	propen_weights_merged_zz_context = t(as.matrix(apply(propen_merged_zz_context, 1, function(x) return(x/sum(x))),
		                                  nrow(propen_merged_zz_context), ncol(propen_merged_zz_context)))
	remove_cells = which(is.na(rowMeans(merged_zz_context)))
	if(length(remove_cells) >= 1){
		merged_zz_context = merged_zz_context[-remove_cells, , drop=F]
		propen_weights_merged_zz_context = propen_weights_merged_zz_context[-remove_cells, , drop=F]
